...
const EnterNewPasscode = (props: any) => {
    ...
    useEffect(() => {
        if (code.length === 4) {
            props.navigation.navigate('ReEnterNewPasscode', { newcode: code });
        }
    }, [code]);
    ...
    return (
        <>
            ...
            <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.WHITE }}>
                <Header navigation={props?.navigation} Heading={t("Enter New Passcode")}
                    HeaderStyle={{ marginLeft: '10%' }} />
                <View style={styles.mainview}>
                    <EnterPasscodesvg />
                    <View style={styles.lineview}>
                        <Text allowFontScaling={false} style={styles.heading}>{t('Enter New Passcode')}</Text>

                        <View style={styles.lineview1}>
                            <Text allowFontScaling={false} style={styles.alltextstyle}>{t('Please enter any 4 digit that will you use to unlock your Telegram app.')}</Text>
                        </View>
                        <OTPInputView
